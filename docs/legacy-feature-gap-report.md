...
| `robctl` / admin command breadth | Old bot offered larger admin command surface (`!throne status/list/search/addsend/addsub/adddomme`, admin HTTP endpoints). | **Partial** | Yes | P1 | v2 `robctl` now includes throne status/dommes/subs, inactivity toggles, blacklist add/remove/list, leaderboard repair/diagnose, and send maintenance tools. |
