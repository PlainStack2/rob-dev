...
def leaderboard_card(*, title: str, entries: list[LeaderboardEntry], summary: LeaderboardSummary, footer: str | None = None, status: str = "🟢 Live"):
    if not entries:
        body = (
            f"-# {status}\n\n"
...
def leaderboard_stats_card(summary: LeaderboardSummary, entries: list[LeaderboardEntry]):
    ...
    return render(make_card(title="🏆 Thy Send Leaderboard | Stats", body=body, color=COLOR_PRIMARY, variant="leaderboard"))
