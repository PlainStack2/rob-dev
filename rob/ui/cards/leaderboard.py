...
def leaderboard_card(*, title: str, entries: list[LeaderboardEntry], summary: LeaderboardSummary, footer: str):
    ...
    return render(make_card(title=title, body=description, color=COLOR_PRIMARY, footer=footer, sections=[CardSection(title="Total Details", text=f"Total Dollar Amount Sent: {format_money_from_cents(summary.total_cents)}\nTotal Dom/mes: {len(top_entries)}")]))
