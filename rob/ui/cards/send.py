...
def send_card(*, send: SendRecord, domme_label: str, sub_label: str | None):
    ...
    return render(make_card(title="Rob | New Send", body=send.item_name or "A new send was logged.", color=COLOR_INFO, footer="Posted from the shared send queue.", sections=[CardSection(title="Domme", text=domme_label, inline=True), CardSection(title="Sender", text=sub_label or "Unclaimed", inline=True), CardSection(title="Amount", text=amount_text, inline=True), CardSection(title="Method", text=send.method or send.source, inline=True), CardSection(title="Sent At", text=format_timestamp(send.sent_at), inline=False)], image_url=send.item_image_url))
