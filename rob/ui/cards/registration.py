...
def registration_card(*, title: str, summary: str, details: list[tuple[str, str]] | None = None) -> RenderedMessage:
    sections = [CardSection(title=name, text=value) for name, value in (details or [])]
    return render(make_card(title=title, body=summary, color=COLOR_SUCCESS, footer=SUCCESS_FOOTER, sections=sections))


def domme_registered_card() -> RenderedMessage:
    return render(
        make_card(
